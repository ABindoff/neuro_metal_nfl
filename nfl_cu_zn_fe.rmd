...
zn <- ggplot(df.zn, aes(x = age, y= fit, colour = strain, group = strain)) +
  ...
  geom_jitter(width = 0.3, aes(x = age, y = logZn), alpha = 0.2) +
  facet_grid(~ region) +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_blank(), 
...
cu <- ggplot(df.cu, aes(x = age, y= fit, colour = strain, group = strain)) +
  geom_line(position = position_dodge(width = 0.1), aes(linetype=strain), size=.6) +
     ...
     geom_errorbar(position = position_dodge(width = 0.1),
                   aes(ymax=upr, ymin=lwr), width=.5) +
        geom_jitter(width = 0.3, aes(x = age, y = Cu), alpha = 0.2) +
  facet_grid(~ region) +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_blank(), 
...
fe <- ggplot(df.fe, aes(x = age, y= fit, colour = strain, group = strain)) +
  geom_line(position = position_dodge(width = 0.2), aes(linetype=strain), size=0.6) +
     geom_point(position = position_dodge(width = 0.1), size=3) + 
     geom_errorbar(position = position_dodge(width = 0.1),
                   aes(ymax=upr, ymin=lwr), width=.5) +
      geom_jitter(width = 0.3, aes(x = age, y = logFe), alpha = 0.2) +
  facet_grid(~ region) +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_blank(), 
...
# profile likelihoods for the variance components

# prof <- profile(fit.fe)
# ci95 <- confint(prof, level = 0.95)
...
Where $\sigma^2_f$ is the fixed-effects variance, $\sigma^2_l$ are the (summed) random variance components, and $\sigma^2_e$ is the residual variance. We also report the *conditional* $R^2$ which includes $\Sigma\sigma^2_l$ in the numerator.
...
$R^2_{logFe} =$ `r round(rABfe,4)`  
$R^2_{logZn} =$ `r round(rABzn,4)`  
$R^2_{Cu} =$ `r round(rABcu,4)`


An estimate of *local effect size* can then be calculated for interaction terms using *Cohen's $f^2$* statistic,    
\[f^2 = \frac{R^2_{AB} - R^2_{A}}{1 - R^2_{AB}}\]  
Where $R^2_{AB}$ is the $R^2$ estimated for the full model (relative to the null model), and $R^2_{A}$ is the $R^2$ estimated for the model without the interaction term of interest. Care must be taken to interpret these effect sizes in context, and they are provided as standardized measures which are of practical benefit in comparing results between studies.
