...
Line plots show the effect of individual mouse. Some animals fairly consistently have higher levels of Zn or Fe, some have consistently lower. Because we cannot control the effect of individual experimentally, we call it a "random effect" and model it as a distribution, rather than a fixed effect (think of the $\beta$ coefficient as being a probability density function rather than a number). This reduces the unexplained variance in the model, resulting in a more powerful model.
