...
Data are first loaded from a .csv file which was prepared using data returned from Florey Institute (Adlard). The following metals were detected at or below detection limits and are excluded from this analysis: Li, Cr,  Co, Ni, Sr, Ru,  Mo, Cd, Ba & Pt. **Some values of Al are negative, possibly due to an adjustment made for Al in the diluent.** Further advice is being sought, but at this stage negative values of Al are set to 1e-323 (as close to zero as a positive number can be within machine precision) so that these values can be log-transformed later.
...
There is a theoretical interest in Fe, Ca, Zn, Al, & RB so plots are produced to investigate these relationships more closely.

```{r exploratory plots2, message = F, warning = F, eval = T}


df0 <- melt(df[,c(1:4, 6:16)], id.vars = c("id", "age", "strain", "region"))
...
There is no reason to model the three-way interaction, so this term is dropped.

```{r MANOVA2, message = F, echo = T}
fit2 <- manova(cbind(logAl, logFe, K, Rb, logCa, logZn) ~ 
                 region + strain + age +
                 region:strain +
                 region:age +
                 strain:age, df)
