...
```{r exploratory plots, message = F, warning = F, eval = F}
...
ggplot(df0, aes(x = strain, y = value, colour = region, alpha = 0.05)) + geom_point() + facet_wrap(~variable, scales = "free", ncol = 3)

```


Further distributional assumptions are checked.  Density plots suggest large departures from normality, driven by outliers. Without *a priori* knowledge of these tests, it is not clear whether these outliers are measurement error (in which case they can be discarded), or reflect a normal biological range. A decision was made to retain outliers and investigate their effect on the models later, particularly as it is likely that a log transformation will be applied.
...
There is no reason to model the three-way interaction, so this term is dropped (this won't change the MANOVA but it will change the Beta coefficients)
